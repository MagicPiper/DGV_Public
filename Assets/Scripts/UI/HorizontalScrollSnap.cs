...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Update()
        {
            if (_lerp)
            {
                _screensContainer.localPosition = Vector3.Lerp(_screensContainer.localPosition, _lerp_target, 17.5f * Time.deltaTime);
        ...
        public void ChangeBulletsInfo(int currentScreen)
        {            
            if (currentScreen > 0)
            {
